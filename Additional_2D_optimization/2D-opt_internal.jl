...
# Flat initial free surface ε
const EPSILON_BACKGROUND = 0.0

# Interface w
const W0_INIT_CONST = -0.150
...
println("\nInitial condition:")
println("  free surface ε is initially flat with ε = $(EPSILON_BACKGROUND)")
println("  initial momenta are zero")
println("  motion is generated only by the initial interface configuration")
...
function setup_twolayer_simulator_2d(; backend=SinFVM.make_cpu_backend(), wprofile)
    ...
    initial = [begin
        ...
        # Zero initial momentum in both layers.
        # Hence the only initial imbalance comes from the interface configuration.
...
println("\nGenerated synthetic observations:")
...
snaps = [
    snapshot(W0_INIT_PROFILE; label="initial guess", t=0.0),
...
ax_conv = Axis(
    fig[1, 1:6],
    title="Convergence: flat initial free surface, interface-driven motion",
    xlabel="iteration",
    ylabel="cost",
    yscale=log10,
)
...
for (j, s) in enumerate(snaps)
    ...
    ax_ε = Axis(
        fig[row, 1],
        title="$(s.label): free surface ε, t=$(round(s.t; digits=2))",
    )
    ...
    ax_w = Axis(
        fig[row, 3],
        title="$(s.label): interface w, t=$(round(s.t; digits=2))",
    )
    ...
    ax_speed = Axis(
        fig[row, 5],
        title="$(s.label): upper-layer speed, t=$(round(s.t; digits=2))",
    )
...
fig_file = joinpath(SAVE_DIR, "optimization_2d_flat_surface_interface_driven.png")
save(fig_file, fig)

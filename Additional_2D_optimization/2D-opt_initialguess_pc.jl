...
#   To what extent may different interface configurations produce similar upper-layer dynamics?
#
# The main idea is to solve the same calibration problem from several different
# initial interface guesses. If the final interfaces differ, but the upper-layer
# velocity fields and objective values are similar, the interface is not uniquely
# identifiable from the chosen upper-layer observations alone.
...
function smooth_random_interface(; seed=1234, mean_value=W0_INIT_CONST, amp=0.02, nsweeps=20)
    rng = MersenneTwister(seed)
    W = mean_value .+ amp .* randn(rng, NX, NY)
    ...
    return project_w_profile(vec(W))
end

function sinusoidal_guess(; mean_value=W0_INIT_CONST, amp=0.02, mode=:x)
    w = Vector{Float64}(undef, NX * NY)
    ...
    for I in eachindex(XY_INT_REF)
        ...
        if mode == :x
            w[I] = mean_value + amp * sin(2π * xhat)
        elseif mode == :y
            w[I] = mean_value + amp * cos(2π * yhat)
        elseif mode == :xy
            w[I] = mean_value + amp * sin(2π * xhat) * sin(2π * yhat)
        elseif mode == :shifted_truth
            w[I] = W0_TRUE_MEAN + amp * sin(2π * (xhat + 0.25)) * cos(2π * yhat)
        else
...
function make_initial_guesses()
    if USE_FULL_GUESS_SET
        return [
            ...
            ("sin_x",     sinusoidal_guess(mean_value=-0.150, amp=0.020, mode=:x)),
            ("sin_y",     sinusoidal_guess(mean_value=-0.150, amp=0.020, mode=:y)),
        ...
        # Smaller default set for thesis diagnostics. These are enough to test
        # whether different-looking interfaces can lead to similar upper-layer flow.
        return [
            ("flat_low",  project_w_profile(fill(-0.180, NX * NY))),
            ("flat_mid",  project_w_profile(fill(-0.150, NX * NY))),
            ("flat_high", project_w_profile(fill(-0.100, NX * NY))),
...
function output_prefix(name)
    return joinpath(SAVE_DIR, "2D_initialguess_" * safe_name(name))
end
...
function save_single_result(r)
    ...
    open(prefix * "_summary.txt", "w") do io
        println(io, "2-D initial guess optimization result")
        println(io, "====================================")
...
function parse_run_indices(n)
    ...
    elseif haskey(ENV, "SLURM_ARRAY_TASK_ID")
        ...
        # Safe default for the PC continuation run: start with task 4.
        # Use "remaining" to run tasks 4--7 sequentially.
        return [4]
    end
end
...
function combine_finished_runs(initial_guesses)
    ...
    results = map(available) do (name, w0, wf)
        ...
        (; name, w0, w_final=wf, J_final=Jf, profile_error=werr, snap=fs)
    end

    sort!(results, by = r -> r.J_final)

    summary_file = joinpath(SAVE_DIR, "2D_initialguess_summary_combined.txt")
    pairwise_file = joinpath(SAVE_DIR, "2D_initialguess_pairwise_combined.txt")

    open(summary_file, "w") do io
        println(io, "2-D multi-start interface identifiability experiment")
        ...
        println(io, "LBFGS_MAX_ITERS = $LBFGS_MAX_ITERS")
        println(io, "")
        @printf(io, "%-14s  %14s  %14s  %14s  %14s  %14s\n",
            "name", "J_final", "w_RMSE_true", "u1_RMSE", "v1_RMSE", "uv_RMSE")

        for r in results
            @printf(
                io,
                "%-14s  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e\n",
                r.name,
                r.J_final,
                r.profile_error,
...
println("USE_FULL_GUESS_SET = $USE_FULL_GUESS_SET")
println("Velocity comparison = directional u1 and v1 components")

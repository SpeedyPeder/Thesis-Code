...
const SAVE_DIR = get(
    ...
    raw"C:\Users\peder\OneDrive - NTNU\År 5\Masteroppgave\Optimization\2-D_Optimization\Predictive Skill",
)
mkpath(SAVE_DIR)
...
function make_initial_guesses()
    # Controlled initial guesses for studying predictive skill from different calibrated interfaces.
...
function save_single_result(r)
    ...
    open(prefix * "_summary.txt", "w") do io
        ...
        println(io, "")
        println(io, "Validation velocity errors outside observation period")
...
function combine_finished_runs(initial_guesses)
    ...
    results = map(available) do (name, w0, wf)
        ...
        (; name, w0, w_final=wf, J0, J_final=Jf, reduction, profile_error=werr,
           snap=fs, train0, trainf, val0, valf)
    end
    ...
    open(pertime_file, "w") do io
        println(io, "Validation errors outside the observation period")
        println(io, "================================================")
    ...
    println("Saved validation-by-time to: $pertime_file")
        ...
        # ---------------------------------------------------------------------
        # Helper for common-colorbar heatmap grids
        # ---------------------------------------------------------------------
        function plot_heatmap_grid_common_colorbar(
            panels,
            values_fun;
            title_prefix,
            colorbar_label,
            ...
            clims = (minimum(all_values), maximum(all_values))
            ...
            for (k, item) in enumerate(panels)
                ...
                hm = heatmap!(ax, field; colorrange=clims)
                hidedecorations!(ax)
            end
        ...
        # ---------------------------------------------------------------------
        # Training/end-of-observation-window panels
        # ---------------------------------------------------------------------
        all_int = [("truth", W0_TRUE_PROFILE)]
        append!(all_int, [(r.name, r.w_final) for r in results])

        int_file = joinpath(SAVE_DIR, "2D_predictive_skill_interfaces_combined.png")
        plot_heatmap_grid_common_colorbar(
            all_int,
            item -> reshape(item[2], NX, NY);
            title_prefix="interface w",
            colorbar_label="w",
            filename=int_file,
        )
        ...
        u1_file = joinpath(SAVE_DIR, "2D_predictive_skill_upperlayer_u1_combined.png")
        plot_heatmap_grid_common_colorbar(
            all_vel,
            item -> item[2].u1;
            title_prefix="upper-layer u₁",
            colorbar_label="u₁",
            filename=u1_file,
        )

        v1_file = joinpath(SAVE_DIR, "2D_predictive_skill_upperlayer_v1_combined.png")
        plot_heatmap_grid_common_colorbar(
            all_vel,
            item -> item[2].v1;
            title_prefix="upper-layer v₁",
            colorbar_label="v₁",
            filename=v1_file,
        )

        # ---------------------------------------------------------------------
        # Future validation panels at final validation time
        # ---------------------------------------------------------------------
        tval = VALIDATION_END
        ...
        all_future = [("truth", truth_future)]
        append!(all_future, future_snaps)

        future_u1_file = joinpath(SAVE_DIR, "2D_predictive_skill_future_u1_combined.png")
        plot_heatmap_grid_common_colorbar(
            all_future,
            item -> item[2].u1;
            title_prefix="future u₁, t=$(round(tval; digits=1))",
            colorbar_label="u₁",
            filename=future_u1_file,
        )

        future_v1_file = joinpath(SAVE_DIR, "2D_predictive_skill_future_v1_combined.png")
        plot_heatmap_grid_common_colorbar(
            all_future,
            item -> item[2].v1;
            title_prefix="future v₁, t=$(round(tval; digits=1))",
            colorbar_label="v₁",
...
println("Predictive diagnostics = training error and validation error outside the observation period")

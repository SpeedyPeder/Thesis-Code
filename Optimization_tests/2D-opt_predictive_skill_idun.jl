...
function combine_finished_runs(initial_guesses)
    ...
    if MAKE_PLOTS
        ...
        # Validation plots at the last future time.
        tval = VALIDATION_END
        ...
        fig_future_u1 = Figure(size=(1800, 900), fontsize=16)
        for (k, (name, snap)) in enumerate(all_future)
            row = div(k - 1, ncols) + 1
            col = mod(k - 1, ncols) + 1
            ax = Axis(fig_future_u1[row, col], title="future u1, t=$(round(tval; digits=1)): $name")
            hm = heatmap!(ax, snap.u1)
            Colorbar(fig_future_u1[row, col + ncols], hm)
            hidedecorations!(ax)
        end
        future_u1_file = joinpath(SAVE_DIR, "2D_predictive_skill_future_u1_combined.png")
        save(future_u1_file, fig_future_u1)
        println("Saved future u1 figure to: $future_u1_file")

        fig_future_v1 = Figure(size=(1800, 900), fontsize=16)
        for (k, (name, snap)) in enumerate(all_future)
            row = div(k - 1, ncols) + 1
            col = mod(k - 1, ncols) + 1
            ax = Axis(fig_future_v1[row, col], title="future v1, t=$(round(tval; digits=1)): $name")
            hm = heatmap!(ax, snap.v1)
            Colorbar(fig_future_v1[row, col + ncols], hm)
            hidedecorations!(ax)
        end
        future_v1_file = joinpath(SAVE_DIR, "2D_predictive_skill_future_v1_combined.png")
        save(future_v1_file, fig_future_v1)
        println("Saved future v1 figure to: $future_v1_file")
    end
end

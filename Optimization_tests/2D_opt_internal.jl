...
function setup_twolayer_simulator_2d(; backend=SinFVM.make_cpu_backend(), wprofile)
    ...
    initial = [begin
        ...
        # The initial dynamics are generated only by the interface configuration.
...
println("  ε scale           = $(EPSILON_SCALE)")
println("  u1 scale          = $(U1_SCALE)")
println("  v1 scale          = $(V1_SCALE)")
...
ax_conv = Axis(
    fig[1, 1:6],
    title="Convergence: periodic flat surface, interface-driven motion",
    xlabel="iteration",
    ylabel="cost",
    yscale=log10,
)
...
fig_file = joinpath(SAVE_DIR, "optimization_2d_periodic_flat_surface_interface_driven.png")
save(fig_file, fig)
